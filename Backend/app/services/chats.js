...
const groupExit = asyncHandler(async (req, res) => {
  ...
  if (isAdmin) {
    return res.status(403).send({status:403,message:"Admin can not leave the group"});
  ...
  if (!removed) {
     return res.status(404).send({status:404,message:"Chat not found"});
...
const addSelfToGroup = asyncHandler(async (req, res) => {
  ...
  if (!added) {
    return es.status(404).send({status:404,message:"hat not found!"});
